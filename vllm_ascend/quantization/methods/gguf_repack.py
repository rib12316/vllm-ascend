...
Q5_0/Q5_1 are 5-bit and don't fit the int4 path, but map to the int8 path (values in
[-16,15] / [0,31] fit int8). k-quants (Q4_K/Q5_K/Q6_K) and IQ variants CANNOT map
(6-bit super-block scales) and keep the dense-dequant fallback. int4 packing uses
``npu_convert_weight_to_int4pack`` (an NPU op); Q8_0/Q5_0/Q5_1 repack is pure torch.
"""
...
from .gguf_dequant import _bytes_to_le_u32

# Block types repacked to the NPU op (high-perf). Others dequant→dense.
GGUF_NPU_REPACK_TYPES = {WT.Q8_0, WT.Q4_0, WT.Q4_1, WT.Q5_0, WT.Q5_1}
