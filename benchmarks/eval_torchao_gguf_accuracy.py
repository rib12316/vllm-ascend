...
def run_lm_eval(label: str, limit: int | None, out_dir: Path,
                eval_timeout: int) -> dict | None:
    ...
    args = [
        ...
        "--tasks", ",".join(ACCURACY_TASKS),
        "--output_path", str(out_dir),
        "--log_samples",
    ]
    if limit is not None:
        args += ["--limit", str(limit)]
    log(f"  lm_eval accuracy: --tasks {','.join(ACCURACY_TASKS)}"
        f"{' --limit ' + str(limit) if limit else ' (full)'}")
    try:
        cp = subprocess.run(args, capture_output=True, text=True,
                            timeout=eval_timeout, cwd=os.getcwd())
...
def parse_results(out_dir: Path) -> dict | None:
    ...
    for task in ACCURACY_TASKS:
        ...
        out[task] = {
            k: round(v, 4)
            for k, v in tr.items()
            if isinstance(v, float) and ("acc" in k or "perplexity" in k)
        }
    return out


def run_config(label: str, limit: int | None, results_root: Path,
               server_timeout: int, eval_timeout: int) -> dict | None:
    model, extra = CONFIGS[label]
    quant = " ".join(a for i, a in enumerate(extra)
                     if i and extra[i - 1] == "--quantization") or "none"
    load_fmt = " ".join(a for i, a in enumerate(extra)
                        if i and extra[i - 1] == "--load_format") or "auto"
    ...
    try:
        if not wait_for_server(proc, server_timeout):
            log(f"  server failed to start within {server_timeout}s "
                f"(see {server_log})")
            return None
...
def main() -> int:
    ...
    ap.add_argument("--configs", default=",".join(CONFIGS),
                    help="comma-separated labels (default: all)")
    ap.add_argument("--limit", type=int, default=None,
                    help="samples per task (default: full)")
    ...
    for label in labels:
        ...
        try:
            all_results[label] = run_config(
                label, args.limit, results_root,
                args.server_timeout, args.eval_timeout,
            )
    ...
    # ---- summary table ----
    log("=== SUMMARY (acc_norm for arc/hellaswag/piqa, acc for winogrande, "
        "both for lambada) ===")
