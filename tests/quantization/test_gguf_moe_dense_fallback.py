...
Weight orientation assumed (R7, verify at real e2e): w13 [E,2*inter,K] is
[out,in] (gate_up = x @ w13[e].T); w2 [E,inter,K] is [in,out] (down = act @ w2[e]).
"""
...
def _make_layer(w13: torch.Tensor, w2: torch.Tensor) -> types.SimpleNamespace:
    ...
    return types.SimpleNamespace(
        w13_qweight=w13.view(torch.uint8),  # [E, 2*inter, K*2] (fp16 → 2 bytes)
        w2_qweight=w2.view(torch.uint8),  # [E, inter, K*2]
...
def _reference_moe(
    ...
    """Pure-torch reference MoE (swiglu) with the ORIGINAL dense weights."""
    inter = w13.shape[1] // 2
    ref = torch.empty_like(x)
    for tok in range(x.shape[0]):
        cur = None
        for w, i in zip(topk_weights[tok], topk_ids[tok]):
            gu = x[tok] @ w13[int(i)].T  # [2*inter]
            g, u = gu[:inter], gu[inter:]
            a = F.silu(g) * u  # [inter]
            d = (a @ w2[int(i)]) * float(w)  # [K]
...
def test_dense_fallback_matches_reference():
    ...
    num_tok, topk = 5, 2
    dtype = torch.float16
    w13 = torch.randn(E, 2 * inter, K, dtype=dtype)
    w2 = torch.randn(E, inter, K, dtype=dtype)
    ...
    x = torch.randn(num_tok, K, dtype=dtype)
...
def test_dense_fallback_single_expert_selection():
    ...
    gu = x @ w13[0].T  # [num_tok, 2*inter]
    g, u = gu[:, :inter], gu[:, inter:]
    ref = (F.silu(g) * u) @ w2[0]  # [num_tok, K]
    torch.testing.assert_close(out, ref, atol=1e-2, rtol=1e-2)
